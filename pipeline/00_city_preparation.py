...
completed stages unless ``--force``):
  ...
  network     pedestrian network from OSM -> clean -> consolidate -> betweenness
              centrality (Bc_Rd) -> dual graph
...
simulation reads (see ``pedsim.core.engine.Import`` / ``Environment``):
  ...
  <City>_buildings.gpkg       buildingID, height, base, land_use, DMA, gScore_sc, lScore_sc
                              (all buildings; landmarks are the runtime subset whose scores
                              pass the thresholds)
...
STAGES = (
    "network", "districts", "elevation", "barriers", "pois",
...
def stage_network(args, stager: Stager) -> None:
    ...
    nodes, edges = ci.consolidate_nodes(nodes, edges, True, args.consolidate_tolerance)
...
def stage_buildings(args, stager: Stager) -> None:
    if stager.done("buildings_structural"):
        log.info("buildings: checkpoint present, skipping")
        return
    ...
    buildings = ci.structural_score(
        buildings, buildings, edges,
        advance_vis_expansion_distance=300, neighbours_radius=150,
    )

    stager.save("buildings_structural", buildings)
...
def stage_sightlines(args, stager: Stager) -> None:
    ...
    buildings = stager.load("buildings_structural")
    ...
    try:
        ...
        log.warning("sightlines: pyvista (visibility3d extra) not installed; skipping stage")
        return
    ...
    log.info("sightlines: computing 3D sight lines (%d nodes x %d targets) — this can take hours",
             len(nodes), len(targets))
    # All our height sources (OSM tags, detailed layer, DEM-DTM) are above-ground heights;
    # 'base' (when the DTM provided it) carries the terrain elevation under the footprint.
    sight_lines = ci.compute_3d_sight_lines(
        nodes, targets, buildings, None, edges, args.city_name,
        distance_along=200, min_observer_target_distance=300,
        height_relative_to_ground=True,
        consolidate=True, consolidate_tolerance=args.consolidate_tolerance,
...
def stage_landmarks(args, stager: Stager) -> None:
    ...
    log.info("landmarks: pragmatic component")
    buildings = ci.pragmatic_score(buildings, search_radius=200)
...
def finalize(args, stager: Stager) -> None:
    ...
    elif stager.path("buildings_structural").exists():
        # No landmark stage yet: still ship the buildings (obstructions, DMA, use tags);
        # the sim runs with landmark navigation disabled.
        stager.load("buildings_structural").pipe(_stringify_list_columns) \
            .to_file(f"{prefix}_buildings.gpkg", driver="GPKG")
...
def parse_args(argv=None):
    ...
    parser.add_argument("--consolidate-tolerance", type=float, default=15.0,
                        help="node consolidation tolerance in metres (default 15)")
