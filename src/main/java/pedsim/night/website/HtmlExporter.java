...
public class HtmlExporter {
  ...
  public static String export(int day, int job, List trips, Map volumesMap) {
    try {
      ...
      String roadsGeoJson =
          pedsim.core.website.GeoJsonExporter.exportRoadsWithVolumes(PedSimCity.roads, volumesMap);
      String tripsJs = buildTripsJs(trips);

      if (isABTestingEnabled()) {
        saveAbTestTrips(tripsJs, job);
      }

      String abTripsJs = loadAbTestTrips(job);
  ...
  private static void saveAbTestTrips(String tripsJs, int job) {
    String path =
        OUTPUT_ROOT
            + File.separator
            + pedsim.core.engine.TripDiagnostic.jobFilename("ab_test_trips.json", job);
    try {
      Files.createDirectories(Paths.get(OUTPUT_ROOT));
  ...
  private static String loadAbTestTrips(int job) {
    String path =
        OUTPUT_ROOT
            + File.separator
            + pedsim.core.engine.TripDiagnostic.jobFilename("ab_test_trips.json", job);
  ...
  private static String renderHtml(
    ...
    return new StringBuilder(HTML_TEMPLATE_1)
        ...
        .replace("__DAY_START__", "6")
        .replace("__NIGHT_START__", "20");
  }
