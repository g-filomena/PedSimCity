...
import pedsim.night.engine.Engine;
import pedsim.night.engine.PedSimCityNight;

/** GUI/headless entry point for the night simulation. */
public class PedSimCityNightApplet extends PedSimCityApplet {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (headless) {
      ...
      pedsim.core.engine.Engine.StateFactory stateFactory = PedSimCityNight::new;
    ...
    if (choice == 2) {
      ...
      preloadDefaultRoadsForDashboard();

      SimulationRestApi.setOnStart(
          params -> {
            try {
              applyDashboardParameters(params);

              ScenarioConfig config =
                  new ScenarioConfig(StringEnum.Vulnerable.values(), StringEnum.TimeOfDay.values());
              new Engine(PedSimCityNight::new).runJobs(config, Pars.jobs > 1);
            } catch (Exception e) {
  ...
  private static void preloadDefaultRoadsForDashboard() {
    try {
      ...
      pedsim.core.engine.SimulationStateStore.getInstance()
          .setRoadsGeoJson(
              pedsim.core.website.GeoJsonExporter.exportRoads(
                  pedsim.core.engine.PedSimCity.roads));
    } catch (Exception e) {
      LoggerUtil.getLogger().warning("Could not pre-load default roads: " + e.getMessage());
    }
  }

  private static void applyDashboardParameters(java.util.Map<String, Object> params) {
  ...
  @Override
  protected pedsim.core.engine.Engine.StateFactory buildStateFactory() {
