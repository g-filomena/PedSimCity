 ...
 *
 * <p>
 * Resolution proceeds as a graded fallback so that constraints are relaxed only
 * as far as necessary:
 * <ol>
 * <li><b>First attempt</b>: avoid parks/water <em>and</em> edges in unknown
 * regions.
 * <li><b>Second attempt</b>: relax the unknown-region constraint but keep
 * avoiding parks/water.
 * <li><b>Final fallback</b>: an unconstrained {@link DijkstraRoadDistance}
 * shortest path.
 * </ol>
 *
 * <p>
 * Both constrained attempts apply only to vulnerable agents; for non-vulnerable
 * agents the filter is a no-op and night avoidance is left to situated
 * navigation.
 *
 * ## Possible to do - add agents filter out zero light edges but add look up
 * table so when they arrive at a node and the start of an edge is dark they
 * reroute
 */
public class DijkstraRoadDistanceNight extends DijkstraRoadDistance {
	 ...
	 *
	 * <p>
	 * The neighbour filter is applied on every attempt; the degree of relaxation is
	 * governed by {@link #secondAttempt} via
	 * {@link #shouldAvoidEdgeAtNight(EdgeGraph, boolean)}.
	 *
	 * <p>
	 * Performance: neighbours are evaluated in a single pass. The shared edge is
	 * resolved once per neighbour and reused for both the avoidance test and the
	 * cost computation (previously it was looked up twice), and no intermediate
	 * stream/list is allocated.
	 *
	 ...
	 *
	 * <p>
	 * The {@code secondAttempt} term is tested first so that, on the relaxed
	 * attempt, the region-knowledge lookup is short-circuited away entirely.
	 *
	 * @param edge          the edge to evaluate
	 * @param secondAttempt if true, relaxes the unknown-region avoidance criterion
	 *                      (parks/water are still avoided)
	 ...
	 * @param edge       the already-resolved edge between the current node and
	 *                   {@code targetNode}
	 ...
	 *
	 * <p>
	 * Performance: each predecessor wrapper is fetched once per step (instead of
	 * three map lookups), and edges are appended then reversed once, avoiding the
	 * O(n^2) cost of repeated head insertions on an {@link ArrayList}.
	 *
	...
	protected List<DirectedEdge> reconstructSequence() {
		...
		if (directedEdgesSequence.isEmpty()) {
			if (!secondAttempt) {
				secondAttempt = true;
				// The relaxed attempt must not inherit dead-ends pruned under the stricter
				// constraints.
