...
public class PedSimCityApplet extends Frame implements ItemListener {
  ...
  static Label jobLabel;
  private static Label remainingTripsLabel;
  private static Label jobsLabel;
  private static Label remainingTripsLabelParallel;
  static int remainingTripsCount;
  private static final Logger LOGGER = Logger.getLogger(Import.class.getName());
  private Thread simulationThread;
  static Display panel;
  // Create and display the panel
  static JFrame frame = new JFrame("Agent Panel");

  /**
   * Constructs a new instance of the `PedSimCityApplet` class, creating a graphical user interface
   * (GUI) applet for configuring and running the PedSimCity simulation. Initialises and arranges
   * various GUI components, including mode selection, city selection, and simulation control
   * buttons.
   */
  ...
  /**
   * Handles changes in the state of the SpecificODcheckbox by either opening or closing the
   * SpecificODpanel.
   *
   * @param e The ItemEvent that triggered the change in the state of the SpecificODcheckbox.
   */
  ...
  /**
   * Opens the `TestPanel, which enables the user to select testing options and parameters.
   */
  ...
  /**
   * Initiates the simulation with the selected parameters and starts the simulation process. This
   * method sets the city name, simulation mode, and other parameters before running the simulation.
   */
  private void startSimulation(boolean runInParallel) {

    TestPars.cityName = cityName.getSelectedItem();
    TestPars.stringMode = modeChoice.getSelectedItem();
    TestPars.defineMode();
  ...
  private void runSimulation(boolean runInParallel) {
    ...
    if (runInParallel) {
      remainingTripsCount =
          TestPars.empirical ? TestPars.numAgents * TestPars.numberTripsPerAgent * TestPars.jobs * 3 // nr
                                                                                     // configurations
              : TestPars.numAgents * TestPars.numberTripsPerAgent * TestPars.jobs;
      updateRemainingTripsLabel(true);
      jobsLabel.setText("Parallelising " + TestPars.jobs + " Jobs");
      jobsLabel.setVisible(true);
      remainingTripsLabelParallel.setVisible(true);

    } else {
      remainingTripsCount = TestPars.empirical ? TestPars.numAgents * TestPars.numberTripsPerAgent * 3 // nr
                                                                                           // configurations
          : TestPars.numAgents * TestPars.numberTripsPerAgent;
  ...
  /**
   * Invoked when an item's state changes. This method handles changes in the state of
   * specificODcheckbox and modeChoice components.
   *
   * @param e an ItemEvent object that provides information about the event (e.g., which item's
   *        state changed).
   */
  @Override
  public void itemStateChanged(ItemEvent e) {
    if (e.getSource() == specificODcheckBox)
      ODpanelStateChanged(e); // Call the ODpanelStateChanged method when specificODcheckbox state
                              // changes
    else if (e.getSource() == modeChoice)
      updateCityNameOptions();
    else
      TestPars.verboseMode = true;
  }
