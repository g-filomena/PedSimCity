...
import pedsim.empirical.agents.EmpiricalAgent;
 ...
 */
public class EmpiricalPopulate extends pedsim.core.engine.Populate {
  ...
  private void assignODMatrixToEmpiricalGroups() {
    ...
    for (EmpiricalAgentsGroup group : EmpiricalPars.empiricalGroups) {
      if (group.groupName == EmpiricalGroup.POPULATION
          && EmpiricalPars.includePopulationBenchmark) {
    ...
    for (EmpiricalAgentsGroup group : EmpiricalPars.empiricalGroups) {
      if (group.groupName != EmpiricalGroup.POPULATION
          && group.groupName != EmpiricalGroup.NULLGROUP) {
  ...
  private int createAgentsForGroup(
      int firstAgentID,
    ...
    for (int i = 0; i < numberAgents; i++) {
      ...
      List<Pair<NodeGraph, NodeGraph>> agentODs =
          new ArrayList<>(groupODMatrix.subList(lowerLimit, upperLimit));
