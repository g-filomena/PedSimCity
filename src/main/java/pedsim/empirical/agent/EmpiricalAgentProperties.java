...
public final class EmpiricalAgentProperties extends AgentProperties {
  ...
  private final List<RouteChoiceProperty> elements =
      Arrays.asList(RouteChoiceProperty.USING_ELEMENTS, RouteChoiceProperty.NOT_USING_ELEMENTS);

  private final List<RouteChoiceProperty> minimisation =
      Arrays.asList(RouteChoiceProperty.ROAD_DISTANCE, RouteChoiceProperty.ANGULAR_CHANGE);

  private final List<RouteChoiceProperty> localHeuristics =
      Arrays.asList(
          RouteChoiceProperty.ROAD_DISTANCE_LOCAL, RouteChoiceProperty.ANGULAR_CHANGE_LOCAL);

  private final List<RouteChoiceProperty> regionBased =
      Arrays.asList(RouteChoiceProperty.REGION_BASED, RouteChoiceProperty.NOT_REGION_BASED);

  private final List<RouteChoiceProperty> subGoals =
      Arrays.asList(
          RouteChoiceProperty.LOCAL_LANDMARKS,
          RouteChoiceProperty.BARRIER_SUBGOALS,
          RouteChoiceProperty.NO_SUBGOALS);

  private final List<RouteChoiceProperty> distantLandmarks =
      Arrays.asList(RouteChoiceProperty.USING_DISTANT, RouteChoiceProperty.NOT_USING_DISTANT);

  private final Map<RouteChoiceProperty, Double> elementsMap =
      new EnumMap<>(RouteChoiceProperty.class);

  private final Map<RouteChoiceProperty, Double> minimisationMap =
      new EnumMap<>(RouteChoiceProperty.class);

  private final Map<RouteChoiceProperty, Double> localHeuristicsMap =
      new EnumMap<>(RouteChoiceProperty.class);

  private final Map<RouteChoiceProperty, Double> regionBasedMap =
      new EnumMap<>(RouteChoiceProperty.class);

  private final Map<RouteChoiceProperty, Double> subGoalsMap =
      new EnumMap<>(RouteChoiceProperty.class);

  private final Map<RouteChoiceProperty, Double> distantLandmarksMap =
      new EnumMap<>(RouteChoiceProperty.class);
  ...
  public void randomizeRouteChoiceParameters() {
    ...
    boolean usingElements =
        weightedChoice(elementsMap, RouteChoiceProperty.NOT_USING_ELEMENTS)
            == RouteChoiceProperty.USING_ELEMENTS;
  ...
  private void setParametersFromGroup() {
    ...
    minimisationMap.put(
        RouteChoiceProperty.ROAD_DISTANCE,
    ...
    localHeuristicsMap.put(
        RouteChoiceProperty.ROAD_DISTANCE_LOCAL,
        sampleProbability(
            group.probabilityLocalRoadDistance, group.probabilityLocalRoadDistanceSD));
    localHeuristicsMap.put(
        RouteChoiceProperty.ANGULAR_CHANGE_LOCAL,
        sampleProbability(
            group.probabilityLocalAngularChange, group.probabilityLocalAngularChangeSD));

    double pRegion =
        sampleProbability(
            group.probabilityRegionBasedNavigation, group.probabilityRegionBasedNavigationSD);
    ...
    double pLocalLandmarks =
        sampleProbability(group.probabilityLocalLandmarks, group.probabilityLocalLandmarksSD);
    double pBarrierSubGoals =
        sampleProbability(group.probabilityBarrierSubGoals, group.probabilityBarrierSubGoalsSD);
    ...
    double pDistant =
        sampleProbability(group.probabilityDistantLandmarks, group.probabilityDistantLandmarksSD);
    ...
    double naturalBarrierScore =
        rescale(
            0.0,
            1.0,
            MIN_NATURAL_BARRIERS,
            MAX_NATURAL_BARRIERS,
            sampleProbability(group.naturalBarriers, group.naturalBarriersSD));
    double severingBarrierScore =
        rescale(
            0.0,
            1.0,
            MIN_SEVERING_BARRIERS,
            MAX_SEVERING_BARRIERS,
  ...
  private void applyLocalHeuristic() {
    RouteChoiceProperty choice =
        weightedChoice(localHeuristicsMap, RouteChoiceProperty.ROAD_DISTANCE_LOCAL);
  ...
  private void activateElements() {
    RouteChoiceProperty regionChoice =
        weightedChoice(regionBasedMap, RouteChoiceProperty.NOT_REGION_BASED);
    setRegionBasedNavigation(regionChoice == RouteChoiceProperty.REGION_BASED);

    RouteChoiceProperty subGoalChoice =
        weightedChoice(subGoalsMap, RouteChoiceProperty.NO_SUBGOALS);
    ...
    RouteChoiceProperty distantChoice =
  ...
  private static void initialiseUniform(
      List<RouteChoiceProperty> properties, Map<RouteChoiceProperty, Double> target) {
  ...
  private RouteChoiceProperty weightedChoice(
      Map<RouteChoiceProperty, Double> weights, RouteChoiceProperty fallback) {
  ...
  private static double rescale(
      double oldMin, double oldMax, double newMin, double newMax, double value) {
