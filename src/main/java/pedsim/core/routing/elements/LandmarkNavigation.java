...
public abstract class LandmarkNavigation implements NavigationElement {
  ...
  protected void idntifyAgentLocalLandmarks() {
    ...
    if (agent == null
        || agent.getHeuristics() == null
        || !agent.getProperties().isUsingLocalLandmarks()) {
      return;
    }
  ...
  protected boolean checkCriteria(NodeGraph candidateNode, double searchDistance) {
    ...
    return !sequence.contains(candidateNode)
        && !candidateNode.equals(originNode)
  ...
  protected boolean checkCriteria(
      NodeGraph candidateNode,
      NodeGraph exitGateway,
      double searchDistance,
      double currentDistance) {

    return !inRegionSequence.contains(candidateNode)
        && !candidateNode.equals(currentNode)
        && network.getEdgeBetween(candidateNode, currentNode) == null
        && GraphUtils.nodesDistance(currentNode, candidateNode) <= searchDistance
  ...
  protected double calculateScore(
      NodeGraph candidateNode, NodeGraph destinationNode, boolean regionBased) {
  ...
  // /**
  // * * Calculates the score for a candidate node based on centrality and
  // distance gain metrics. *
  // * * @param candidateNode The node being evaluated. * @param minCentrality The
  // minimum
  // centrality
  // * value in the network. * @param maxCentrality The maximum centrality value
  // in the network.
  // * * @return The calculated score for the candidate node, considering
  // centrality and distance
  // * gain.
  // */
  // protected double calculateScore(NodeGraph candidateNode, double
  // minCentrality,
  ...
  // : (candidateNode.getCentrality() - minCentrality) / (maxCentrality -
  // minCentrality);
  // double currentDistance = GraphUtils.nodesDistance(currentNode,
  // destinationNode);
  // double distanceGain =
  // (currentDistance - GraphUtils.nodesDistance(candidateNode, destinationNode))
  // / currentDistance;
  ...
  //
  // /**
  // * Calculates the score for a candidate node based on centrality/landmarkness
  // and distance gain.
  // *
  ...
  // */
  // protected double calculateScore(NodeGraph candidateNode, NodeGraph target,
  // double
  // currentDistance,
  ...
  //
  // double distanceGain =
  // (currentDistance - GraphUtils.nodesDistance(candidateNode, target)) /
  // currentDistance;
  //
  ...
  // List<Building> nodeLocalLandmarks = new
  // ArrayList<>(candidateNode.adjacentBuildings);
  ...
  //
  // /**
  // * Computes the global landmarkness score for a target node based on the
  // global landmarks in its
  ...
  // */
  // public static double globalLandmarknessNode(NodeGraph targetNode, NodeGraph
  // destinationNode) {
  //
  // // get the distant landmarks
  // List<Building> distantLandmarks = new
  // ArrayList<>(targetNode.visibleBuildings3d);
  //
  ...
  // double targetDistance = GraphUtils.nodesDistance(targetNode,
  // destinationNode);
  ...
  // new
  // ArrayList<>(LandmarkIntegration.getDistances(destinationNode).getArray());
