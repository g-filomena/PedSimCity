...
public class Dijkstra {
	...
	protected void computeTentativeCostDual(NodeGraph currentNode, NodeGraph targetNode, double turnCost) {
		...
		if (landmarkCondition(targetNode)) {
			double globalLandmarkness = Landmarkness.globalLandmarknessDualNode(currentNode, targetNode,
					finalDestinationNode);
			double nodeLandmarkness = 1.0 - globalLandmarkness * agent.getHeuristics().getGlobalLandmarkWeight(true);
	...
	private boolean landmarkCondition(NodeGraph targetNode) {
		return (!properties.shouldOnlyUseMinimization() && properties.isUsingDistantLandmarks()
				&& GraphUtils.nodesDistance(targetNode, finalDestinationNode) > RouteChoicePars.threshold3dVisibility);
	}
	...
	protected boolean regionCondition() {
		return properties.isRegionBasedNavigation() && originNode.getRegionID() == destinationNode.getRegionID()
				&& agent.getCognitiveMap().isRegionKnown(originNode.getRegionID());
	}

	protected boolean isRegionKnown(int regionID) {
		return agent.getCognitiveMap().isRegionKnown(regionID) || SharedCognitiveMap.isRegionKnownByCommunity(regionID);
	}
