...
public class SharedCognitiveMap {
  ...
  private static void setCommunityPrimalNetwork(Graph network) {
    ...
    communityNetwork
        ...
        .forEach(
            edge ->
                Pars.roadTypes.entrySet().stream()
                    .filter(
                        entry ->
                            Arrays.asList(entry.getValue())
                                .contains(edge.attributes.get("roadType").getString()))
                    .findFirst()
                    .ifPresent(entry -> roadTypeMap.put(edge, entry.getKey())));
  ...
  private static void prepareCommunityKnownEdges() {
    ...
    communityKnownNodes.addAll(
        communityKnownEdges.stream()
            .flatMap(edge -> edge.getNodes().stream())
            .collect(Collectors.toSet()));

    // add salient nodes
    Set<NodeGraph> salientNodes =
        new HashSet<>(
  ...
  private static void buildRoadClassification() {
    ...
    primaryEdges =
        roadTypeMap.entrySet().stream()
            .filter(entry -> entry.getValue() == RoadType.PRIMARY)
            .map(Map.Entry::getKey)
            .collect(Collectors.toSet());

    secondaryEdges =
        roadTypeMap.entrySet().stream()
            .filter(entry -> entry.getValue() == RoadType.SECONDARY)
            .map(Map.Entry::getKey)
            .collect(Collectors.toSet());

    tertiaryEdges =
        roadTypeMap.entrySet().stream()
            .filter(entry -> entry.getValue() == RoadType.TERTIARY)
            .map(Map.Entry::getKey)
            .collect(Collectors.toSet());

    neighbourhoodEdges =
        roadTypeMap.entrySet().stream()
            .filter(entry -> entry.getValue() == RoadType.NEIGHBOURHOOD)
            .map(Map.Entry::getKey)
            .collect(Collectors.toSet());
  ...
  private static void setCommunityBarriers() {
    ...
    PedSimCity.barriersMap.forEach(
        (barrierID, barrier) -> {
