...
public class BarrierIntegration {
  ...
  public HashMap<Geometry, Set<Integer>> intersectingBarriers(
      NodeGraph currentLocation, NodeGraph destinationNode, Agent agent) {

    HashMap<Geometry, Set<Integer>> viewFieldIntersectingBarriers = new HashMap<>();
    Set<Integer> intersectingBarrierIDs = new HashSet<>();
    AgentBarrierType agentBarrierCategory = agent.getProperties().getBarrierType();
    ...
    Predicate<Integer> isBarrierRelevant =
        barrierID -> {
          ...
          if (agentBarrierCategory.equals(AgentBarrierType.NEGATIVE)) {
            return barrierType.equals(BarrierType.RAILWAY)
                || barrierType.equals(BarrierType.ROAD)
    ...
    intersectingGeometries.stream()
        .map(g -> g.getIntegerAttribute("barrierID")) // Extract
        // barrierID
  ...
  public static void setEdgeGraphBarriers(EdgeGraph edge) {
    ...
    Map<String, List<Integer>> attributesMap =
        Map.of(
            "p_barr",
            positiveBarriers,
            "n_barr",
    ...
    attributesMap.forEach(
        (key, list) -> {
          ...
          if (!value.equals("[]")) {
            Arrays.stream(value.replaceAll("[^-0-9]+", " ").trim().split(" "))
                .filter(s -> !s.isBlank())
                .map(Integer::valueOf)
                .forEach(list::add);
          }
          ...
          edge.attributes.put(
              key.replace("p_barr", "positiveBarriers")
                  .replace("n_barr", "negativeBarriers")
                  .replace("a_rivers", "waterBodies")
                  .replace("w_parks", "parks"),
