...
/**
 * The Complexity class computes wayfinding easiness and legibility complexity for navigation. It
 * includes methods for calculating wayfinding easiness within a space or region based on distance
 * and landmarks.
 */
public class Complexity {

  /**
   * Computes the wayfinding easiness within a space between two nodes on the basis of: a) the ratio
   * between the distance between the passed nodes and a certain maximum distance; b) the legibility
   * of the space, based on the presence of landmarks.
   *
  ...
  public double wayfindingEasiness(NodeGraph node, NodeGraph destinationNode, Agent agent) {
    ...
    double distanceComplexity = GraphUtils.nodesDistance(node, destinationNode)
    ...
    List<MasonGeometry> buildings =
        new ArrayList<>(agent.getCognitiveMap().getBuildings(node, destinationNode));
  ...
  /**
   * Computes the wayfinding easiness within a region on the basis of: a) the ratio between the
   * distance that would be walked within the region considered and the distance between the origin
   * and the destination; b) the legibility of the region based on the presence of landmarks.
   *
  ...
  public double wayfindingEasinessRegion(NodeGraph currentNode, NodeGraph exitGateway,
      NodeGraph originNode, NodeGraph destinationNode, Agent agentProperties) {
  ...
  /**
   * Computes the building-based complexity of a region based on the type of landmarkness (local or
   * global).
   *
  ...
  public double buildingsRegionComplexity(Region region, Agent agent) {
    ...
    // TODO
    List<MasonGeometry> buildings =
        new ArrayList<>(SharedCognitiveMap.getBuildingsWithinRegion(region));
    if (buildings.isEmpty()) {
      return 1.0;
    }
    ...
    if (agentLandmarkType.equals(LandmarkType.LOCAL)) {
      landmarks = new ArrayList<>(agent.getCognitiveMap().getRegionLocalLandmarks(region));
    } else {
      landmarks = new ArrayList<>(SharedCognitiveMap.getRegionGlobalLandmarks(region));
    }
  ...
  /**
   * Retrieves landmarks for the agent based at the city or region level (if provided).
   *
  ...
  public List<MasonGeometry> getAgentLandmarks(Agent agent, Region region) {
    ...
    LandmarkType type = agent.getProperties().landmarkType;

    if (region != null) {
      return (type == LandmarkType.LOCAL) ? agent.getCognitiveMap().getRegionLocalLandmarks(region)
          : SharedCognitiveMap.getRegionGlobalLandmarks(region);
    }
    return (type == LandmarkType.LOCAL)
        ? SharedCognitiveMap.getLocalLandmarks()
            .getGeometriesFromIDs(agent.getCognitiveMap().getLocalLandmarksIDs())
        : SharedCognitiveMap.getGlobalLandmarks().getGeometries();
  }
}
