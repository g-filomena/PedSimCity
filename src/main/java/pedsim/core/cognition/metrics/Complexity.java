...
public class Complexity {
	...
	public double wayfindingEasiness(NodeGraph node, NodeGraph destinationNode, Agent agent) {
		...
		List<MasonGeometry> buildings = new ArrayList<>(agent.getCognitiveMap().getBuildings(node, destinationNode));
	...
	public double buildingsRegionComplexity(Region region, Agent agent) {
		...
		// TODO
		List<MasonGeometry> buildings = new ArrayList<>(SharedCognitiveMap.getBuildingsWithinRegion(region));
		if (buildings.isEmpty()) {
			return 1.0;
		}

		LandmarkType agentLandmarkType = agent.getProperties().getLandmarkType();
	...
	public List<MasonGeometry> getAgentLandmarks(Agent agent, Region region) {
		...
		LandmarkType type = agent.getProperties().getLandmarkType();
