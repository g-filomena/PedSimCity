...
public class obsolete extends SimState
{
    ...
    HashMap<MasonGeometry, Double> nodesRC =  new HashMap<MasonGeometry, Double>();
...
//    HashMap<Double, HashMap<MasonGeometry, Double>> nodesCDF =  new HashMap<Double, HashMap<MasonGeometry, Double>>();
    ...
    String criteria[] = {"euclidean", "angular", "topological",  "euclideanLand", "angularLand", "landmark","landmarkL", "landmarkG" };
    ...
    public int numTripsScenario = 118;
    ...
    public obsolete(long seed, int job)
    {
        super(seed);
...
//    	String [][] visibilityMatrix; 
    ...
    public void saveCSV() throws IOException
    {
		...
		Bag rGeometries = roads.getGeometries();
	   	String csvSegments = "C:/Users/g_filo01/sciebo/GIS Data/Simulation/LondonTest_pedestrian_counts_120_single_"+current_job+".csv";
//	   	String csvSegments = "C:/Users/g_filo01/sciebo/GIS Data/Simulation/LondonTest_pedestrian_counts_120_"+current_job+".csv";
		FileWriter writerSegmentsData = new FileWriter(csvSegments);
//	    CSVUtils.writeLine(writerSegmentsData, Arrays.asList(
//	    		"streetID", "euclidean", "angular", "topological", "euclideanLand","angularLand", "landmark", "landmarkL", "landmarkG"));
	    CSVUtils.writeLine(writerSegmentsData, Arrays.asList(
	    		"streetID", "euclidean", "angular", "topological", "euclideanLand","angularLand", "landmark"));
...
//	   	String csvStatistics = "try.csv";
//		FileWriter writerStatistics = new FileWriter(csvStatistics);
//		
//		
//	    CSVUtils.writeLine(writerStatistics, 
//	    		Arrays.asList("criteria", "trips", "avgDistance", "avgDiffEuclidean", "avgDiffLandmark"));
//	    
//	    for (int i = 0; i < criteria.length; i++) 
//	    {
//	        criteriaData cd = criteriaMap.get(criteria[i]);
//	        CSVUtils.writeLine(writerStatistics, Arrays.asList(criteria[i], Integer.toString(cd.trips), Double.toString(cd.totalDistance)));
//  		}
//	    
...
//            String [][] visibilityMatrix = null;
//            if (visibility == true)
//            {
//	            System.out.println("reading visiblity matrix...");
//	            visibilityMatrix = new String[buildings.getGeometries().size()+1][geometriesNodes.size()+1];
////	            CSVReader reader = new CSVReader(new FileReader("C:/Users/g_filo01/sciebo/Tools/Mason/sim/app/geo/London/data/London_visibility_matrix.csv"));
//	            
//	            CSVReader reader = new CSVReader(new FileReader("C:/Users/g_filo01/sciebo/Tools/Mason/sim/app/geo/London/data/London_visibility_matrix.csv"));
//	            String [] nextLine;
//	            
...
//            	double Bc400 = geometryNode.getDoubleAttribute("Bc_400"); 
//            	Integer station = geometryNode.getIntegerAttribute("station"); 
//            	double Bc = geometryNode.getDoubleAttribute("Bc"); 
//            	double Rc = geometryNode.getDoubleAttribute("Rc600");
...
//    }
    ...
    public void populate()
    {
    	...
    	for (int i = 0; i < numTripsScenario; i++)
    	{
    		...
    		while ((startingNode == null) | (control.contains(startingNode))) startingNode = nodesLookup.searchRandomNode(geometriesNodes, this);
    		GeometryFactory fact = new GeometryFactory();
    		control.add(startingNode);
    		MasonGeometry nodeLocation = new MasonGeometry(fact.createPoint(new Coordinate(startingNode.getCoordinate())));
    		Node destinationNode = nodesLookup.searchNodeWithin(nodeLocation.geometry, junctions, 1900, 2100, this);
    		Pair<Node, Node> pair = new Pair<Node, Node> (startingNode, destinationNode);
    	    OD.add(pair);  
    	}
        ...
        {   
        	...
        	else if (i == 5) routeCriteria = "angularLand";
        	else if (i == 6) routeCriteria = "landmark";
        	else if (i == 7) routeCriteria = "landmarkL";
        	else routeCriteria = "landmarkG";
        	
//        	int c = random.nextInt(7);      	
//        	String routeCriteria = criteria[c];
        	
//        	int r = random.nextInt(4);
//        	Integer maxDistance = radium[r];
        	
//        	Integer maxDistance = 12000;
//         	PedestrianLondon a = new PedestrianLondon(this, startingNode, routeCriteria, i, maxDistance); //, goalEdge );
...
//            junctionsAttributes.add("Bc"); 
	        ...
	        URL roadsFile = obsolete.class.getResource("data/London_edges_simplified.shp");
	        URL junctionsFile = obsolete.class.getResource("data/London_nodes_simplified.shp");
            ShapeFileImporter.read(roadsFile, roads);
            ShapeFileImporter.read(junctionsFile, junctions, junctionsAttributes);
            ...
            Bag centroidsAttributes = new Bag();
            centroidsAttributes.add("streetID");
            URL roadsDualFile = obsolete.class.getResource("data/London_edgesDual_simplified.shp");
            URL centroidsFile = obsolete.class.getResource("data/London_nodesDual_simplified.shp");            
            ShapeFileImporter.read(roadsDualFile, intersectionsDual);
            ShapeFileImporter.read(centroidsFile, centroids, centroidsAttributes);
            ...
            buildingsAttributes.add("lScore_sc");  
            buildingsAttributes.add("gScore_sc");  
            URL landmarksFile = obsolete.class.getResource("data/London_landmarks.shp");
            ShapeFileImporter.read(landmarksFile, buildings);
...
//          readerDistances.close();
	    	...
	    	for (int i = 0; i < jobs; i++)
	    	{
		    	...
		    	SimState state = new obsolete(System.currentTimeMillis(), i); 
		    	state.start(); 
...
//    	doLoop(London.class, [-repeat 2]);
//    	System.out.println("finished "+i);
//    	}
////    	System.exit(0);
//    	do
//    		if (!state.schedule.step(state)) break;
////    	while(state.schedule.getSteps() < 2); 
////    	state.finish(); 
