...
public class NightAgentMovement extends pedsim.core.agents.AgentMovement {
  ...
  /**
   * Every city edge that is both outside the community-known network and reads as non-lit for a
   * non-vulnerable agent: the fixed lighting half of a non-vulnerable agent's avoid-set (register
   * finding C3). "Non-lit" here means the same thing {@code NightBehaviour}'s gate means after the
   * C1 fix &mdash; continuous {@code mean_lux} (and {@code min_lux}, where present) below {@link
   * NightPars#nonVulnerableLightSensitivity}, falling back to the raw OSM {@code lit} tag only
   * where no continuous data exists at all &mdash; instead of the raw tag unconditionally, which
   * is what {@code SharedCognitiveMap.getEdgesNonLitNonCommunityKnown()} still returns.
   *
   * <p>Built here in the night module rather than by changing that core method: every non-vulnerable
   * agent shares the same fixed {@code nonVulnerableLightSensitivity}, so this can stay the single
   * static, network-wide set the original was, computed once and cached exactly like {@link
   * #edgesOutsideCommunityKnown()} above &mdash; without giving {@code pedsim.core.cognition.cognitivemap}
   * a dependency on {@code pedsim.night}, which importing {@link NightPars} into that class would
   * have. The vulnerable-agent avoid-set is untouched: it's built from community-vs-agent
   * knowledge, not lighting, and the register left whether that's the intended design to the code
   * owner.
   */
  private static volatile Set<EdgeGraph> nonVulnerableNonLitNonCommunityKnown;

  private static Set<EdgeGraph> nonVulnerableNonLitNonCommunityKnown() {
    Set<EdgeGraph> cached = nonVulnerableNonLitNonCommunityKnown;
    if (cached == null) {
      double threshold = NightPars.nonVulnerableLightSensitivity;
      Set<EdgeGraph> knownEdges = SharedCognitiveMap.getCommunityKnownEdges();
      cached = new HashSet<>();
      for (EdgeGraph edge : SharedCognitiveMap.getCommunityPrimalNetwork().getEdges()) {
        if (knownEdges.contains(edge)) {
          continue;
        }
        if (!isLitForNonVulnerable(edge, threshold)) {
          cached.add(edge);
        }
      }
      nonVulnerableNonLitNonCommunityKnown = cached;
    }
    return cached;
  }

  /**
   * Whether an edge counts as lit for a non-vulnerable agent: continuous {@code mean_lux} (and
   * {@code min_lux}, where present) at or above {@code threshold}, or -- for an edge with no
   * continuous lux data at all -- the raw OSM {@code lit} tag. Mirrors {@code
   * NightBehaviour.meanLightPasses}'s fallback shape exactly, so this avoid-set and the gate that
   * actually triggers a non-vulnerable reroute agree on what "lit" means.
   */
  private static boolean isLitForNonVulnerable(EdgeGraph edge, double threshold) {
    var meanLuxAttr = edge.attributes.get("mean_lux");
    if (meanLuxAttr == null) {
  ...
  public static void clearCachedNetworkSets() {
    ...
    nonVulnerableNonLitNonCommunityKnown = null;
  }
   ...
   * is unlit and not community-known &mdash; via {@link #nonVulnerableNonLitNonCommunityKnown()}, a
   * lux-based set (register finding C3), not {@code
   * SharedCognitiveMap.getEdgesNonLitNonCommunityKnown()}'s raw-flag one; see that method's Javadoc
   * for why. Parks and water are added for the vulnerable, and for anyone currently avoiding them.
   *
  ...
  private void defineEdgesToAvoid() {
    ...
    if (agent.isVulnerableBoolean()) {
      ...
      edgesToAvoid.addAll(nonVulnerableNonLitNonCommunityKnown());
    }
