 ...
 *
 * <p><b>Known limitation &mdash; no transit.</b> {@link #step} fully overrides {@code
 * ActivityAgent.step()} without calling {@code super}, so every night trip is walked end to end;
 * the transit-boarding branch {@code ActivityAgent} otherwise reaches is never called from this
 * class, and there are zero references to {@code pedsim.transit} anywhere under {@code
 * pedsim.night}. This is deliberate, not an oversight left open to fix later: the existing
 * transit-boarding code assigns riders from three hardcoded archetype lines and walks an
 * all-stops loop at one simulation tick per hop, which would distort night trips more than
 * walking them in full does &mdash; enabling it as-is would trade one bias for a worse one
 * (register finding C6).
 *
 * <p><b>Direction of the bias this leaves in place:</b> the phone-data validation this module is
 * checked against (Torino Vodafone Smart Locator; Lyon's comptage-mobilites counters) counts
 * metro and bus riders alongside pedestrians. Any real trip that would have been partly ridden is
 * modelled here as fully walked. So reported night walking volumes, trip distances, and
 * lighting-exposure metrics (see {@link #getTripMeanLux}) are a conservative <em>upper bound</em>
 * on real pedestrian street exposure, not a direct estimate of it &mdash; the bias runs heavier
 * on trips that parallel transit corridors (where a real traveller has an alternative to walking
 * the whole way) than on trips with no nearby transit option, so it is not a uniform inflation
 * across the network.
 */
public class NightAgent extends ActivityAgent {
   ...
   *
   * <p>Intentionally does not call {@code super.step()}, and so never reaches {@code
   * ActivityAgent}'s transit-boarding branch &mdash; see the class-level note above for why, and
   * what that leaves biased in the reported metrics.
   */
  @Override
   ...
   *
   * <p>Walk-only (see the class-level note on transit): this figure is exposure per metre actually
   * walked in the simulation, which for any trip a real traveller would have partly ridden covers
   * more street metres — and therefore more opportunity to cross a dark one — than that traveller
   * really experienced.
   */
  @Override
