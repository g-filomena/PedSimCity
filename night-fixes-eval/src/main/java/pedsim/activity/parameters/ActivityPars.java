...
public class ActivityPars {
  ...
  /**
   * Share of the 15-24 band in employment. {@link pedsim.activity.agents.Persona#sample} thins the
   * student share by it so the employed young are counted once, as workers.
   *
   * <p><b>A national order of magnitude, not a Turin figure</b> - ISTAT's Italian 15-24 employment
   * rate has run in the high teens; Piedmont and a city rate are higher. Set the local value in
   * {@code <City>.properties}.
   */
   ...
   * match an observed distribution, rather than being handed one. At the value below the utility
   * cost of a kilometre is about 1.3 points, which is roughly the pull of a node with three times
   * the attraction -- the same order of magnitude the original 0.0012 was chosen at; see the
   * consistency note below for why it isn't 0.0012 any more.
   *
   ...
   *
   * <p><b>Consistency fix, not a re-calibration (register Section F, "distanceWeight is stale for
   * Torino's real circuity").</b> {@link pedsim.activity.agents.DestinationChoice#choose} scales
   * straight-line distance by {@code Pars.networkCircuityFactor} before applying this coefficient
   * (so it means "utility per metre <i>walked</i>", not per metre as the crow flies), and only the
   * <i>product</i> {@code distanceWeight * networkCircuityFactor} actually sets the choice
   * probabilities -- the method's own docstring: "the scaling is absorbed into the coefficient".
   * {@code 0.0012} was set while {@code networkCircuityFactor} sat at its old fallback, 1.41. Circuity
   * is now self-measured per city by default ({@code Pars.measureNetworkCircuity}) and comes out
   * 1.292 for Torino, but this coefficient was never re-derived to match -- so the product, and with
   * it the trip-length distribution this coefficient exists to shape, silently drifted 8.4% the
   * moment circuity self-measurement landed, with nobody having decided that.
   *
   * <p>Re-derived by holding the product this coefficient and the old fallback circuity produced
   * invariant against the now-measured value: {@code 0.0012 * (1.41 / 1.292) ~= 0.0013096}. This
   * restores the shape the model already produced before self-measurement changed its one input,
   * rather than asserting a new one -- it is not a fit against Audimob or any other observed
   * distribution, which is still the open request the class-level Javadoc above describes.
   *
   * <p>Scoped to Torino specifically: this is one global constant multiplying one
   * self-measured-per-city circuity value, not a per-city dynamic correction. A different city's
   * measured circuity would need its own re-derivation the same way -- {@code
   * ActivityAgent.decideCommuteMode()}/{@code commuteMetres()} and {@code CommuteCalibration}
   * already read {@code Pars.networkCircuityFactor} live at call time instead of baking a value in,
   * which is the more general pattern and needs no equivalent fix; folding {@code DestinationChoice}
   * into that same live-reference pattern would remove this per-city caveat entirely, but is a
   * larger change than this one-line consistency fix and wasn't made here.
   */
  public static double distanceWeight = 0.0013096;
