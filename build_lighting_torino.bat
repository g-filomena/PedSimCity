...
REM   2) city file prefix before _edges.gpkg, _puntiLuce.gpkg, etc.
...
set /p CITY_NAME=Enter city file prefix ^(before _edges.gpkg / _puntiLuce.gpkg, e.g. Torino^): 
if "%CITY_NAME%"=="" (
...
if not exist "%INPUT_DIR%\%CITY_NAME%_puntiLuce.gpkg" (
    echo Expected puntiLuce file not found:
    echo "%INPUT_DIR%\%CITY_NAME%_puntiLuce.gpkg"
    pause
    exit /b 1
)
...
echo Building street lighting ^(Turin puntiLuce^) in folder: "%INPUT_DIR%"
echo Using city file prefix: "%CITY_NAME%"

"%CONDA_EXE%" run --no-capture-output -n "%ENV_NAME%" python "%ROOT%pipeline\build_lighting_torino.py" --input_dir "%INPUT_DIR%" --city_name "%CITY_NAME%"
